...
% is a figure with the Precision-Recall curve.
    ...
    [F1sorted, F1sorted_idx] = sort(F1score);
    
    labels = cellfun(@num2str,num2cell(F1sorted_idx), ...
                     'uniformoutput', 0);
    fig = ShowPrecisionRecallCurve(labels, TP(F1sorted_idx), ...
                                   FP(F1sorted_idx), FN(F1sorted_idx));
end