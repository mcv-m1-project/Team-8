function def_Candidates = check_candidates(Candidates,step,margin)
    def_Candidates=[];
        ...
        for i=1:size(Candidates,1)-1          
               ...
               %if(keepCand == true)
                    ...
                    for j=1:length(list_equals)
                        if(((Candidates(i,3)) >= (Candidates(list_equals(j),3) - margin)) & ((Candidates(i,3)) <= (Candidates(list_equals(j),3) + margin)))
%                             if Candidates(i,3)~=Candidates(list_equals(j),3)
%                                 Candidates(i,3)
%                                 Candidates(list_equals(j),3)
%                             end
...
%     if size (def_Candidates)~=size(Candidates)
%        size(Candidates)
%        size(def_Candidates)
%        bueaus
%     end
